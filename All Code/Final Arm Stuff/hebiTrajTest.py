...
target_xyz = np.array([-0.3, 0, 0.42])

target_pose = getIK(model, target_xyz, getFbk(group).position)
...
fig = mlab.figure(size = (1920, 720), bgcolor = (0, 0, 0))


p1 = mlab.points3d(X[:, 0], X[:, 1], X[:, 2], color = (0.2, 0.2, 0.2), scale_factor=.02)
l1 = mlab.plot3d(X[:, 0], X[:, 1], X[:, 2], tube_radius = 0.005)
#p2 = mlab.points3d(Y[:, 0], Y[:, 1], Y[:, 2], color = (0.2, 0.2, 0.2), scale_factor=.02)
#l2 = mlab.plot3d(Y[:, 0], Y[:, 1], Y[:, 2], tube_radius = 0.005)
ax1 = mlab.plot3d(np.array([-q, q]), np.array([0, 0,]), np.array([0, 0]), tube_radius = 0.002, color = (1, 0, 0))
ax2 = mlab.plot3d(np.array([0, 0]), np.array([-q, q]), np.array([0, 0]), tube_radius = 0.002, color = (0, 1, 0))
ax3 = mlab.plot3d(np.array([0, 0]), np.array([0, 0,]), np.array([-q, q]), tube_radius = 0.002, color = (0, 0, 1))
t = mlab.points3d(target_xyz[0], target_xyz[1], target_xyz[2], scale_factor=.02, color = (0, 0.5, 1))
...
animation = mpy.VideoClip(make_frame, duration=T).resize(0.1)
animation.write_videofile("armTest.avi", fps=np.int(1/Ts), audio=False, threads = 10, codec='mpeg4')
#animation.write_gif("wireframe.gif", fps=30,  opt='OptimizePlus')
# for j in range(N):
#     t = j*Ts
#     pos_cmd, vel_cmd, acc_cmd = traj.get_state(t)
#     setCmd(group, cmd, getFbk, pos_cmd, vel_cmd, acc_cmd)
